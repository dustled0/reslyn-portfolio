    ...
    <section id="skills" className="py-20 bg-slate-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-14">
          {skillCategories.map((cat, i) => (
            <FadeIn key={cat.title} delay={i * 0.08}>
              <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 hover:border-amber-500/40 transition-colors h-full">
        ...
        <FadeIn className="text-center">
          <h3 className="text-slate-400 font-medium mb-5 text-sm tracking-widest uppercase">
            Languages
          </h3>
          <div className="flex flex-wrap justify-center gap-3">
            {languages.map((l) => (
              <div
                key={l.lang}
                className="bg-slate-800 border border-slate-700 px-6 py-2.5 rounded-full"
              >
                <span className="text-white font-medium">{l.lang}</span>
                <span className="text-slate-400 text-sm ml-2">&middot; {l.level}</span>
              </div>
            ))}
          </div>
        </FadeIn>
