    ...
    <section id="skills" className="py-20 bg-emerald-950">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-14">
          {skillCategories.map((cat, i) => (
            <FadeIn key={cat.title} delay={i * 0.08}>
              <div className="bg-emerald-900 rounded-2xl p-6 border border-emerald-800 hover:border-amber-500/40 transition-colors h-full">
                <div className="text-3xl mb-3">{cat.icon}</div>
                <h3 className="font-bold text-white text-lg mb-4">{cat.title}</h3>
                <ul className="space-y-2">
                  {cat.skills.map((skill) => (
                    <li key={skill} className="flex items-center gap-2 text-emerald-200 text-sm">
        ...
        <FadeIn className="text-center">
          <h3 className="text-emerald-400 font-medium mb-5 text-sm tracking-widest uppercase">
            Languages
          </h3>
          <div className="flex flex-wrap justify-center gap-3">
            {languages.map((l) => (
              <div
                key={l.lang}
                className="bg-emerald-900 border border-emerald-800 px-6 py-2.5 rounded-full"
              >
                <span className="text-white font-medium">{l.lang}</span>
                <span className="text-emerald-300 text-sm ml-2">&middot; {l.level}</span>
              </div>
            ))}
          </div>
        </FadeIn>
