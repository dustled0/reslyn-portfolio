            ...
            {contacts.map((c, i) => (
              <FadeIn key={c.label} delay={i * 0.1}>
                <a
                  ...
                  className={`relative bg-slate-800 border border-slate-700 ${c.hoverColor} rounded-2xl p-6 text-center transition-all group flex flex-col items-center h-full`}
                >
                  {c.badge && (
          ...
          <FadeIn className="flex justify-center gap-3">
            {profiles.map((p) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`flex items-center gap-2 bg-slate-800 ${p.color} border border-slate-700 text-slate-300 hover:text-white font-medium px-5 py-2.5 rounded-xl text-sm transition-all`}
              >
                {p.icon}
                {p.label}
              </a>
